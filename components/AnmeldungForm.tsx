  ...
  return (
    <form onSubmit={handleSubmit} className="rounded-[28px] bg-white p-8 shadow-[0_10px_30px_rgba(0,0,0,0.06)]">
      <div className="grid gap-5 md:grid-cols-2">
        ...
        <label className="grid gap-2 font-bold">Alter des Kindes<input className={inputClass} name="alter" /></label>
        <label className="grid gap-2 font-bold md:col-span-2">Gewünschter Start<input className={inputClass} name="start" /></label>
        <label className="grid gap-2 font-bold md:col-span-2">Nachricht<textarea className={`${inputClass} min-h-36 resize-y`} name="nachricht" /></label>
      </div>
      <div className="mt-7 flex flex-col items-start gap-4 sm:flex-row sm:items-center">
        <Button type="submit">
          Anfrage senden <Send aria-hidden="true" size={20} />
        </Button>
        {sent ? <p className="font-bold text-[#3F7B43]">Vielen Dank! Ihre Anfrage wurde vorbereitet.</p> : null}
      </div>
    </form>
  );
}
